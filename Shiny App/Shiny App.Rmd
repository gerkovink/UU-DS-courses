...
server = function(input, output) { 
  ...
  #click=reactive(input$click_event)
  output$d3 <- renderD3({
...
output$table <- DT::renderDataTable({
  ...
  if(is.null(input$click_event)){
    DS %>% filter(Faculty=="Faculty of Social Sciences",Level==level(),`Course type`==course()
                  #eval(parse(text=input$click_event))==1
                  ) %>% select(c(1,3,5,7,8,9,12))
  }else{
    
    DS %>% filter(Faculty=="Faculty of Social Sciences",Level==level(),`Course type`==course(),
                  (!!sym(input$click_event))==1
                  ) %>% select(c(1,3,5,7,8,9,12))
  }
