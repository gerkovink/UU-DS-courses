...
server = function(input, output, session) { 
  ...
  mina=reactive({input$min_courses-1})
...
#input$min_courses
  for (i in 1:length(faculty_names)) {
    ...
    local({
      ...
      data <- DS %>% select(c(1,4,6,19:60)) %>% filter(Faculty==faculty_names[i]) %>% pivot_longer(where(is.numeric)) %>% group_by(name) %>% summarise(value=sum(value,na.rm =T))

      output[[plotname]] <- renderD3({
        data<- data%>% filter(value>(input$min_courses-1))
    colnames(data)<-c("id","value")
    data<-left_join(data,short, by="id")
        r2d3(data=data, d3_version = 4, script ="bubble.js")})
    })
